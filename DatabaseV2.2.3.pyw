...
Version V2.2.4   | Αλλαγή Directory απο Dropbox σε GOOGLE-DRIVE  --------------------------------------04/12/2020
...
Version V2.0.6   | Διευθυνση και ΤΗΛ στους Πελάτες ==> Scolledtext-------------------------------------------01/01/2020
...
if sys.platform == "linux":
    ...
    dbase = "/home/dannys/qnap/GOOGLE-DRIVE/ΕΓΓΡΑΦΑ/2.  ΑΠΟΘΗΚΗ/3. ΚΑΙΝΟΥΡΙΑ_ΑΠΟΘΗΚΗ.db"
else:
    # ML Shop dbases
    dbase = "\\\\192.168.1.200\\Public\\GOOGLE-DRIVE\\ΕΓΓΡΑΦΑ\\2.  ΑΠΟΘΗΚΗ\\3. ΚΑΙΝΟΥΡΙΑ_ΑΠΟΘΗΚΗ.db"
...
def get_tables():
    ...
    cursor.close()
    conn.close()
    dont_used_tables = ["sqlite_master", "sqlite_sequence", "sqlite_temp_master", "Images"]  # Πινακες που δεν θέλουμε
...
class Toplevel1:
    def __init__(self, top=None):
        ...
        self.empty_button.configure(image=self.empty_button_img)
        self.empty_button.configure(compound="left")
        ...
        self.del_button_img = PhotoImage(file="icons/delete_selected.png")
    ...
    def change_color(self, table_name):
        ...
        if table_name == self.tables[-1]:
            ...
            self.empty_button.place(relx=0.701, rely=0.240, height=34, width=180)

            self.del_button.configure(background="red", foreground="white")
            self.del_button.place(relx=0.880, rely=0.240, height=34, width=180)
    ...
    def edit(self):  # Επεξεργασία δεδομένων
        ...
        colors = ["MAGENTA", "YELLOW", "CYAN", "BLACK", "C/M/Y"]
        yspot = 0.080
        big_entry = ["ΠΕΡΙΓΡΑΦΗ", "ΠΑΡΑΤΗΡΗΣΕΙΣ", "ΔΙΕΥΘΥΝΣΗ"]
        for index, header in enumerate(self.headers):
            if header == "ID" or header == "id" or header == "Id":
                continue
            else:
                ...
                elif header == "ΔΙΕΥΘΥΝΣΗ":
                    address = ScrolledText(edit_window, height=3, border=2)
                    address.insert("1.0", selected_data[index])
                    address.place(relx=0.130, rely=yspot, relheight=0.100, relwidth=0.400)
                    data_to_add.append(address)
                    yspot += 0.050

                elif header == "ΤΗΛ":
                    phone = ScrolledText(edit_window, height=3, border=2)
                    phone.insert("1.0", selected_data[index])
                    phone.place(relx=0.130, rely=yspot, relheight=0.100, relwidth=0.400)
                    data_to_add.append(phone)
                    yspot += 0.050
    ...
    def add_files(self):
        ...
        self.files = filedialog.askopenfilenames(initialdir=os.getcwd(), title="Επιλογή αρχείων για προσθήκη",
                                                 filetypes=[("Υπ. αρχεία", "*.jpg *.png *.jpeg *.pdf")])
        ...
        if search_data.get() != "":  # Αν έχουμε γράψει κάτι στην αναζήτηση στο search_entry
            ...
            for data in fetch:
                ...
                if odd_or_even % 2 == 0 and color:
                    ...
                    self.Scrolledtreeview.insert("", "end", values=data,
                                                 tags=(
                                                     "evenrow" + color[0] if len(color) < 2 else "evenrow" + color[
                                                         -1],))
                else:
